...
Handlebars.registerHelper('sy_time', function(time_string, style){
  time_string = moment(time_string).zone(480).format('h:mm A');
  return time_string;
});
...
<span class="sy-heading">Event Date : </span>
{{sy_date calendar.start_date}} 


{{#if calendar.start_time }}
<br>
<span class="sy-heading">Event Time :</span>
<span class="s sytime">{{sy_time calendar.start_time }}</span>
{{/if}}

{{#if calendar.end_time }}
to <span class="s sytime">{{sy_time calendar.end_time }}</span>
{{/if}}
...
{{#if calendar.end_time }}
<span class="s sytime">{{sy_time calendar.end_time }}</span>
{{/if}}
